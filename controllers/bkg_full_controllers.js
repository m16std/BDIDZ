...
class controller {
    ...
    async findFullBkg(req, res) {
        const select = await db.query('SELECT * FROM booking LEFT JOIN rsd_list ON booking.bkg_num = rsd_list.bkg_num LEFT JOIN resident ON rsd_list.rsd_pasport = resident.rsd_pasport WHERE bkg_num = $1 ORDER BY booking.bkg_num', [req.body.bkg_num])
        const data = select.rows;
        res.render('../pages/bkg_full_page', { data });
    }
}
